...
#   print(paste("i= ", i))
  ...
  if (any(var.EBPrcAllsim1 ==0)){
    ...
    R[i] <- sum(resultEBPrcAllsim1$Q.values[[3]] <=.05)
    S[i] <- sum(which(resultEBPrcAllsim1$Q.values[[3]] <=.05)>simdat$EE)
    FDR[i] <- 1 - S[i]/max(R[i],1)
    print(paste0('FDR= ', FDR[i], ", S = ", S[i], ", R = ", R[i]))
    resultsim1[[i]] <- resultEBPrcAllsim1
  }
  # save(var.EBPrcAllsim1, file = "var.EBPrcAllsim1.RData")
  # load(file = "var.EBPrcAllsim1.RData")
  
}
save(resultsim1, file = "resultsim1.RData")
save(R, file = "R.RData")
save(S, file = "S.RData")
save(FDR, file = "FDR.RData")
