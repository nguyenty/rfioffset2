...
#load("~/research/rfi/pvalue0511/pi0_0.7/simdat_1.RData")
pm1 <- proc.time()
...
# dir <- "~"
R7sim1 <- S7sim1 <- FDR7sim1 <- ind7sim1 <- NULL
result7sim1 <- list()
# resultsim1[[1]] <- 1
# resultsim1[[2]] <- 2
for (i in 1:100){# i <- 21
#   print(paste("i= ", i))
  path <- paste0("~/research/rfi/pvalue0511/pi0_0.7/simdat_", i,".RData")
  #  path <- paste0("P:/research/rfi/pvalue0511/pi0_0.7/simdat_", i,".RData")
  #  path <- paste0(dir,"/research/rfi/pvalue0511/pi0_0.7/simdat_", i,".RData")
  load(path)
  y <- simdat$y
  ...
  if (any(var.EBPrcAllsim1 ==0)){
    ind7sim1[i] <- 0
    print(paste("i = ", i, ", ind7sim1 = ", ind7sim1[i]))
  } else{
    ind7sim1[i] <- 1
    print(paste("i = ", i, ", ind7sim1 = ", ind7sim1[i]))
    ...
    R7sim1[i] <- sum(resultEBPrcAllsim1$Q.values[[3]] <=.05)
    S7sim1[i] <- sum(which(resultEBPrcAllsim1$Q.values[[3]] <=.05)>simdat$EE)
    FDR7sim1[i] <- 1 - S7sim1[i]/max(R7sim1[i],1)
    print(paste0('FDR= ', FDR7sim1[i], ", S = ", S7sim1[i], ", R = ", R7sim1[i]))
    result7sim1[[i]] <- resultEBPrcAllsim1
  }
  # save(var.EBPrcAllsim1, file = "var.EBPrcAllsim1.RData")
  # load(file = "var.EBPrcAllsim1.RData")
  
}
save(result7sim1, file = "result7sim1.RData")
save(R7sim1, file = "R7sim1.RData")
save(S7sim1, file = "S7sim1.RData")
save(FDR7sim1, file = "FDR7sim1.RData")
save(ind7sim1, file = "ind7sim1.RData")
proc.time() -pm1
