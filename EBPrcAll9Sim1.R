...
#load("~/research/rfi/pvalue0511/pi0_0.9/simdat_1.RData")
pm1 <- proc.time()
...
# dir <- "~"
R9sim1 <- S9sim1 <- FDR9sim1 <- ind9sim1 <- NULL
result9sim1 <- list()
# resultsim1[[1]] <- 1
# resultsim1[[2]] <- 2
for (i in 1:100){# i <- 21
#   print(paste("i= ", i))
  path <- paste0("~/research/rfi/pvalue0511/pi0_0.9/simdat_", i,".RData")
  #  path <- paste0("P:/research/rfi/pvalue0511/pi0_0.9/simdat_", i,".RData")
  #  path <- paste0(dir,"/research/rfi/pvalue0511/pi0_0.9/simdat_", i,".RData")
  load(path)
  y <- simdat$y
  ...
  if (any(var.EBPrcAllsim1 ==0)){
    ind9sim1[i] <- 0
    print(paste("i = ", i, ", ind9sim1 = ", ind9sim1[i]))
  } else{
    ind9sim1[i] <- 1
    print(paste("i = ", i, ", ind9sim1 = ", ind9sim1[i]))
    ...
    R9sim1[i] <- sum(resultEBPrcAllsim1$Q.values[[3]] <=.05)
    S9sim1[i] <- sum(which(resultEBPrcAllsim1$Q.values[[3]] <=.05)>simdat$EE)
    FDR9sim1[i] <- 1 - S9sim1[i]/max(R9sim1[i],1)
    print(paste0('FDR= ', FDR9sim1[i], ", S = ", S9sim1[i], ", R = ", R9sim1[i]))
    result9sim1[[i]] <- resultEBPrcAllsim1
  }
  # save(var.EBPrcAllsim1, file = "var.EBPrcAllsim1.RData")
  # load(file = "var.EBPrcAllsim1.RData")
  
}
save(result9sim1, file = "result9sim1.RData")
save(R9sim1, file = "R9sim1.RData")
save(S9sim1, file = "S9sim1.RData")
save(FDR9sim1, file = "FDR9sim1.RData")
save(ind9sim1, file = "ind9sim1.RData")
proc.time() -pm1
